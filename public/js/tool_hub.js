...
class IcosahedronGeometry extends Geometry {
    ...
    generateIcosahedron() {
        ...
        this.vertices = positions.map((pos, i) => new Vertex(pos, i));
        const faceIndices = [
        ...
        this.faces = faceIndices.map(indices => new Face(indices.map(i => this.vertices[i])));
    }
    subdivide() {
        const newFaces = [];
        ...
        const getMidpoint = (v1, v2) => {
            ...
            const midVertex = new Vertex(midPos, this.vertices.length);
        ...
        for (const face of this.faces) {
            ...
            newFaces.push(new Face([v1, m1, m3]), new Face([v2, m2, m1]), new Face([v3, m3, m2]), new Face([m1, m2, m3]));
        }
        this.faces = newFaces;
    }
    normalizeVertices() {
        for (const vertex of this.vertices) {
            const len = Math.sqrt(vertex.position[0] ** 2 +
                vertex.position[1] ** 2 +
                vertex.position[2] ** 2);
...
class InfiniteGridMenu {
    constructor(canvas, items = []) {
        ...
        this.render = () => {
            ...
            const viewMatrix = Mat4.lookAt(Mat4.create(), this.camera.position, this.camera.target, this.camera.up);
    ...
    destroy() {
        ...
        if (this.gl) {
            ...
            if (this.instancePositionBuffer)
                this.gl.deleteBuffer(this.instancePositionBuffer);
            if (this.instanceScaleBuffer)
                this.gl.deleteBuffer(this.instanceScaleBuffer);
            if (this.instanceRotationBuffer)
                this.gl.deleteBuffer(this.instanceRotationBuffer);
            if (this.instanceTextureBuffer)
                this.gl.deleteBuffer(this.instanceTextureBuffer);
            if (this.vao)
...
function initInfiniteMenu(canvasElement, toolsData) {
...
window.initInfiniteMenu = initInfiniteMenu;
